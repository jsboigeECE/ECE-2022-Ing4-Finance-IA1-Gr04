...
using System.Linq;
using Sudoku.SudokuSolver.Core;

namespace Sudoku.SolverHumain
{
    class SolverHumain : ISolverSudoku
    {
        public GridSudoku Solve(GridSudoku s)
        {
            int res;
            int[][] board = s.Cellules;
            Puzzle p = new Puzzle(board, true);

            Console.WriteLine("\nChoose Solver: \n1 - Humain \n2 - Humain + Backtracking\n");
            ...
            s.Cellules = p.GetBoard();
            return s;
        }

        public Puzzle Solve1(Puzzle s)
        {
            ...
            foreach (var i in System.Linq.Enumerable.Range(0, 9))
            {
                foreach (var j in System.Linq.Enumerable.Range(0, 9))
                {
                    s.SetCellule(i, j, monPuzzle.Rows[i][j].Value);
        ...
        public Puzzle Solve2(Puzzle s)
        {
            GridSudoku ogGridSudoku = new GridSudoku();
            ogGridSudoku.Cellules = s.GetBoard();
            ...
            do
            {
                ...
                if (!full)
                {
                    ...
                    {
                        GridSudoku grid = new GridSudoku();
                        grid.Cellules = monPuzzle.GetBoard();
                        //Plus de candidats possibles, on atteint un cul-de-sac
                        if (grid.IsValid(ogGridSudoku))
                        {
                            solved = true;
                        }
                        else
            ...
            foreach (var i in System.Linq.Enumerable.Range(0, 9))
            {
                foreach (var j in System.Linq.Enumerable.Range(0, 9))
                {
                    s.SetCellule(i, j, monPuzzle.Rows[j][i].Value);
